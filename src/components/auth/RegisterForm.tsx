...
import { faEnvelope, faLock, faSpinner, faUser, faCalendarDays, faShieldHalved } from '@fortawesome/free-solid-svg-icons';
...
const RegisterForm: React.FC = () => {
  ...
  // Шаг 1: Валидация формы и отправка кода
  ...
  const handleVerifyAndRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (data?.user) {
          try {
            console.log('Updating profile with:', { first_name: firstName.trim(), date_of_birth: dateOfBirth });
